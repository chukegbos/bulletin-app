...
@section('pageTitle', 'Edit Publication')
@section('content')
        ...
        <div class="col-md-12">
          <div class="box box-solid">
            ...
            <div class="box-body">
              ...
              <form role="form" enctype="multipart/form-data" action="{{ url('admin/editpost') }}/{{ $post->id}}" method="post" >
                  {{ csrf_field() }}
                  {{ method_field('PUT') }}
                  <div class="row">
                    ...
                    <div class="col-md-3 col-sm-3 col-xs-12" style="border: 1px solid grey;margin-right:2px"> 
                      <div style="margin-top:10px; margin-right:10px">
                        <input type="hidden" name="author" value="{{ Auth::user()->name }}">
                              ...
                              <fieldset>
                                  ...
                                  <div style="border: 1px solid grey; height: 20em; overflow-y: auto; white-space: nowrap; padding:5px">
                                      ...
                                      <div class="c-inputs-stacked">
                                        @if(in_array($category->title, $exploded)) 
                                          <input checked type="checkbox" id="checkbox_{{ $category->title }}" name="category[]" value="{{ $category->title }}"/>
                                          <label for="checkbox_{{ $category->title}}" class="block"> {{ $category->title }}</label> 
                                        @else
                                          <input type="checkbox" id="checkbox_{{ $category->title }}" name="category[]" value="{{ $category->title }}"/>
                                          <label for="checkbox_{{ $category->title}}" class="block"> {{ $category->title }}</label> 
                                        @endif
                                        ...
                                        <div style="margin-left:30px"> 
                                          @forelse($subs as $sub)
                                            @if($category->title == $sub->parent)
                                              @if(in_array($sub->title, $exploded)) 
                                                <input checked type="checkbox" id="checkbox_{{ $sub->title }}" name="category[]" value="{{ $sub->title }}"/>
                          ...
                          <div class="form-group">
                            @if(isset($post->pdf))
                              <embed width="250" height="207" name="plugin" src="{{ asset('storage') }}/{{ $post->pdf }}" type="application/pdf">
                            @endif
                            <label>Re-Add PDF</label>
                            <input class="form-control" value="{{ $post->tag }}" type="file"  name="pdf"/>
                          </div>
